...
// In a production app, these values would come from environment variables.
// Frontend build tools (like Vite, Next.js, Create React App) expose these
// variables to the client-side code. They often require a specific prefix, like `VITE_` or `REACT_APP_`.
//
// Example for a Vite project: process.env.VITE_INSTAGRAM_APP_ID
//
// The fallback 'YOUR_INSTAGRAM_APP_ID' is for development convenience.
...
export const getInstagramAuthUrl = (): string => {
  if (!INSTAGRAM_APP_ID || INSTAGRAM_APP_ID === 'YOUR_INSTAGRAM_APP_ID') {
    // In a real app, you might throw an error or disable the button if the App ID isn't configured.
    console.error("Instagram App ID is not configured. Please set it in your environment variables.");
